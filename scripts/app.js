...
let afk = {
    ...
    warnTimeout: 110,   // The time to elapse before warning the user they are inactive.
    closeTimeout: 15,   // The time after the warning when we disconnect the user.
...
function showConnectOverlay() {
    ...
    setOverlay('clickableState', startText, event => {
        connect();
        //startAfkWarningTimer(); // Start AFK after player's queuePos got updated
    });
}
...
function stopAfkWarningTimer() {
    afk.active = false;
    /* ****** */
    if (afk.warnTimer != undefined)
    {
        clearTimeout(afk.warnTimer);
        afk.warnTimer = undefined;
    }
}
...
function registerMouseEnterAndLeaveEvents(playerElement) {
    ...
    playerElement.onmouseleave = function(e) {
        ...
        {
            /* ****** */
            // Comment it out to prevent from mouse disappearing
            // when cursor move out of browser
            //sendInputData(Data.buffer);
...
function connect() {
    ...
    ws.onclose = function(event)
    {
        ...
        document.getElementById('queuePos-icon').setAttribute('src', 'images/Disconnected.png');
        document.getElementById('queuePos-pos').innerHTML = '';
        document.getElementById('queuePos-count').innerHTML = '';
    };
}
...
function UE5HandleResponseFunction(data) {
    switch (data) {
        case "PlayerDead":
            ...
            // Send event to let UE know not to show SwitchPlayer Widget
