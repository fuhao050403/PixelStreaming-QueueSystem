...
playerServer.on('connection', function (ws, req) {
	...
	players.set(playerId, { ws: ws, id: playerId, queuePos: playerCount - 1 }); /* add queuePos property */
	...
	function sendQueuePos(leftPlayerPos)
	{
		for (let p of players.values())
		{
	...
	ws.on('message', (msgRaw) =>{
		...
		/* ****** */
		// else if (msg.type == 'joinQueueRequest')
		// {
		// 	players.get(playerId).queuePos = queueLength;
		// 	queueLength++;
		// 	sendQueuePos(-1);
		// }
	...
	function onPlayerDisconnected() {
		try {
			--playerCount;
			/* ****** */
			let dcUserQeueuPos = players.get(playerId).queuePos;
			/* ****** */
